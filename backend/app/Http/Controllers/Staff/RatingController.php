...
class RatingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        CustomerRating::where('branch_id', $staff->branch_id)
            ->whereNull('viewed_at')
        ...
        $stats = [
            'total_ratings' => CustomerRating::where('branch_id', $staff->branch_id)->count(),
            'average_rating' => CustomerRating::where('branch_id', $staff->branch_id)->avg('rating') ?? 0,
            'five_star' => CustomerRating::where('branch_id', $staff->branch_id)->where('rating', 5)->count(),
            'four_star' => CustomerRating::where('branch_id', $staff->branch_id)->where('rating', 4)->count(),
            'three_star' => CustomerRating::where('branch_id', $staff->branch_id)->where('rating', 3)->count(),
            'two_star' => CustomerRating::where('branch_id', $staff->branch_id)->where('rating', 2)->count(),
            'one_star' => CustomerRating::where('branch_id', $staff->branch_id)->where('rating', 1)->count(),
        ];

        return view('staff.ratings.index', compact('customersWithRatings', 'stats'));
    }
    ...
    public function show(CustomerRating $rating)
    {
        ...
        // Ensure rating belongs to staff's branch
        if ($rating->branch_id !== $staff->branch_id) {
            abort(403, 'Unauthorized: This rating belongs to a different branch.');
        }

        $rating->load(['customer', 'branch']);
        
        return view('staff.ratings.show', compact('rating'));
    }
}