...
class NotificationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = BranchNotification::where('branch_id', $branch->id)
        ...
        $counts = [
            'all' => BranchNotification::where('branch_id', $branch->id)->count(),
            'unread' => BranchNotification::where('branch_id', $branch->id)->whereNull('read_at')->count(),
            'read' => BranchNotification::where('branch_id', $branch->id)->whereNotNull('read_at')->count(),
        ];

        // Get notification types for filter
        $types = BranchNotification::where('branch_id', $branch->id)
            ->distinct()
            ->pluck('type')
            ->filter()
            ->values();

        return view('branch.notifications.index', compact('notifications', 'counts', 'types'));
    }
    ...
    public function getRecent(Request $request)
    {
        ...
        $notifications = BranchNotification::where('branch_id', $branch->id)
            ->orderBy('created_at', 'desc')
            ->limit($limit)
            ->get()
    ...
    public function getUnreadCount()
    {
        ...
        $count = BranchNotification::where('branch_id', $branch->id)
            ->whereNull('read_at')
            ->count();
    ...
    public function markAsRead($id)
    {
        ...
        $notification = BranchNotification::where('branch_id', $branch->id)
            ->findOrFail($id);
    ...
    public function markAllAsRead()
    {
        ...
        BranchNotification::where('branch_id', $branch->id)
            ->whereNull('read_at')
            ->update([
                'read_at' => now(),
            ]);
    ...
    public function deleteAllRead()
    {
        ...
        $deleted = BranchNotification::where('branch_id', $branch->id)
