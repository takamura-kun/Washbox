...
<head>
    ...
    <meta name="description" content="WashBox Branch Dashboard">
    <title>@yield('page-title', 'Dashboard') - WashBox Branch</title>
...
<body data-theme="light" data-role="branch">
    <!-- Hidden data container for JavaScript -->
    <div id="notificationSystem"
         data-unread-count-url="{{ route('branch.notifications.unread-count') }}"
         data-recent-url="{{ route('branch.notifications.recent') }}"
         data-mark-read-url="{{ url('branch/notifications') }}"
         data-mark-all-read-url="{{ route('branch.notifications.mark-all-read') }}"
         style="display: none;"></div>

    <!-- Sidebar -->
    @include('branch.components.sidebar')
        ...
        <div class="topbar">
            <div class="topbar-left">
                ...
                <nav class="breadcrumb-nav" aria-label="breadcrumb">
                    <a href="{{ route('branch.dashboard') }}" class="breadcrumb-item">Dashboard</a>
                ...
                <div class="user-menu-wrapper">
                    <div class="user-menu dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                        <div class="user-avatar" id="userAvatar">
                            {{ strtoupper(substr(auth()->guard('branch')->user()->name ?? 'B', 0, 1)) }}
                        </div>
                        <div class="user-info">
                            <span class="user-name">{{ auth()->guard('branch')->user()->name ?? 'Branch' }}</span>
                            <span class="user-role">Branch</span>
                        </div>
                        <i class="bi bi-chevron-down"></i>
                    </div>
                    <ul class="dropdown-menu dropdown-menu-end shadow border-0 rounded-3">
                        <li>
                            <a class="dropdown-item py-2" href="{{ route('branch.profile') }}">
                                <i class="bi bi-person"></i>Profile
                            </a>
                        </li>
                        <li>
                            <a class="dropdown-item py-2" href="{{ route('branch.notifications.index') }}">
                        ...
                        <li>
                            <form id="logoutForm" method="POST" action="{{ route('branch.logout') }}" class="mb-0">
                                @csrf
