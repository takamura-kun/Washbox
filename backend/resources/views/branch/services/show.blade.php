...
<div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('branch.services.index') }}" class="btn btn-outline-secondary">
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-4">
            {{-- Service Card --}}
            <div class="card border-0 shadow-sm rounded-4 mb-4" style="background-color: var(--surface);">
                <div class="card-body p-4 text-center" style="color: var(--text-1);">
                    {{-- Image/Icon --}}
                    <div class="mb-3">
                    ...
                    {{-- Name & Type --}}
                    <h5 class="fw-bold mb-1" style="color: var(--text-1);">{{ $service->name }}</h5>
            ...
            {{-- Service Details --}}
            <div class="card border-0 shadow-sm rounded-4" style="background-color: var(--surface);">
                <div class="card-header py-3" style="background-color: var(--surface); border-bottom: 1px solid var(--border);">
                    <h6 class="mb-0 fw-bold" style="color: var(--text-1);">
                        <i class="bi bi-info-circle me-2 text-primary"></i>
                        Service Details
                    </h6>
                </div>
                <div class="card-body p-4" style="color: var(--text-1);">
                    @if($service->description)
                        <div class="mb-3 pb-3 border-bottom" style="border-bottom-color: var(--border) !important;">
                            <small class="text-muted d-block mb-1">Description</small>
                            <p class="mb-0" style="color: var(--text-1);">{{ $service->description }}</p>
                        </div>
                    @endif

                    <div class="mb-3 pb-3 border-bottom" style="border-bottom-color: var(--border) !important;">
                        <small class="text-muted d-block mb-1">Turnaround Time</small>
                        <strong class="d-block" style="color: var(--text-1);">{{ $service->turnaround_time }} hours</strong>
                    </div>

                    @if($service->min_weight || $service->max_weight)
                    <div class="mb-3 pb-3 border-bottom" style="border-bottom-color: var(--border) !important;">
                        <small class="text-muted d-block mb-1">Weight Limits</small>
                        <strong class="d-block" style="color: var(--text-1);">
                            @if($service->min_weight && $service->max_weight)
                    ...
                    <div class="mb-3 pb-3 border-bottom" style="border-bottom-color: var(--border) !important;">
                        ...
                        <strong class="d-block" style="color: var(--text-1);">{{ $service->created_at->format('F d, Y') }}</strong>
                        <small class="text-muted">{{ $service->created_at->diffForHumans() }}</small>
                    </div>

                    <div>
                        <small class="text-muted d-block mb-1">Last Updated</small>
                        <strong class="d-block" style="color: var(--text-1);">{{ $service->updated_at->format('F d, Y') }}</strong>
        ...
        <div class="col-lg-8">
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-4">
                    <div class="card border-0 shadow-sm rounded-4 h-100" style="background-color: var(--surface);">
                        <div class="card-body p-3 text-center" style="color: var(--text-1);">
                            <div class="bg-primary bg-opacity-10 p-2 rounded-3 d-inline-block mb-2">
                                <i class="bi bi-arrow-repeat fs-4 text-primary"></i>
                            </div>
                            <h4 class="fw-bold mb-0" style="color: var(--text-1);">{{ $times_used }}</h4>
                ...
                <div class="col-md-4">
                    <div class="card border-0 shadow-sm rounded-4 h-100" style="background-color: var(--surface);">
                        <div class="card-body p-3 text-center" style="color: var(--text-1);">
                            ...
                            <h4 class="fw-bold mb-0" style="color: var(--text-1);">₱{{ number_format($total_revenue, 2) }}</h4>
                ...
                <div class="col-md-4">
                    <div class="card border-0 shadow-sm rounded-4 h-100" style="background-color: var(--surface);">
                        <div class="card-body p-3 text-center" style="color: var(--text-1);">
                            ...
                            <h4 class="fw-bold mb-0" style="color: var(--text-1);">{{ number_format($total_revenue / max($times_used, 1), 2) }}</h4>
            ...
            <div class="card border-0 shadow-sm rounded-4" style="background-color: var(--surface);">
                <div class="card-header py-3" style="background-color: var(--surface); border-bottom: 1px solid var(--border);">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('branch.laundries.index', ['service_id' => $service->id]) }}" class="btn btn-sm btn-outline-secondary">
                            View All
                        </a>
                    </div>
                </div>
                <div class="card-body p-0" style="color: var(--text-1);">
                    ...
                    <div class="p-3 border-bottom {{ $loop->last ? 'border-0' : '' }}" style="border-bottom-color: var(--border) !important;">
                        <div class="d-flex justify-content-between align-items-start">
                            <div>
                                <strong class="d-block mb-1" style="color: var(--text-1);">#{{ $laundry->tracking_number ?? $laundry->id }}</strong>
                            ...
                            <div class="text-end">
                                ...
                                <small class="fw-semibold text-primary" style="color: var(--text-1);">₱{{ number_format($laundry->total_amount, 2) }}</small>
                            </div>
                        </div>
                    </div>
                    @empty
