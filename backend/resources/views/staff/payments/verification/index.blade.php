...
<div class="pv-page">
    ...
    <div class="pv-header">
        ...
        <div class="pv-stats">
            <div class="pv-stat-pill pill-warning">
                <span class="stat-number">{{ $paymentProofs->where('status','pending')->count() }}</span>
                <span class="stat-label">Pending</span>
            </div>
            <div class="pv-stat-pill pill-success">
                <span class="stat-number">{{ $paymentProofs->where('status','approved')->count() }}</span>
                <span class="stat-label">Approved</span>
            </div>
            <div class="pv-stat-pill pill-danger">
                <span class="stat-number">{{ $paymentProofs->where('status','rejected')->count() }}</span>
                <span class="stat-label">Rejected</span>
            </div>
            <div class="pv-stat-pill">
                <span class="stat-number">{{ $paymentProofs->total() }}</span>
    ...
    <div class="pv-filter-card">
        ...
        <div class="pv-filter-pills">
            ...
            <a href="{{ route('staff.payments.verification.index', ['status' => 'pending']) }}"
               class="pv-pill-btn pill-pending {{ request('status') === 'pending' ? 'active' : '' }}">
                <span class="dot"></span> Pending
            </a>
            <a href="{{ route('staff.payments.verification.index', ['status' => 'approved']) }}"
               class="pv-pill-btn pill-approved {{ request('status') === 'approved' ? 'active' : '' }}">
                <span class="dot"></span> Approved
            </a>
            <a href="{{ route('staff.payments.verification.index', ['status' => 'rejected']) }}"
    ...
    <div class="pv-table-card">
        <div class="pv-table-header">
            ...
            @if($paymentProofs->where('status', 'pending')->count() > 0)
                ...
                <div class="card border-0 shadow-sm rounded-4 h-100" style="background: var(--pv-surface); border: 2px solid var(--pv-border) !important; transition: transform 0.2s, box-shadow 0.2s;">
                    <div class="card-body p-4">
                        ...
                        <div class="d-flex gap-2">
                            <a href="{{ route('staff.payments.verification.show', $proof) }}" class="btn btn-primary flex-fill" style="border-radius: 10px; font-size: 0.85rem; padding: 0.65rem; font-weight: 600;">
                                <i class="bi bi-receipt"></i> View Details
                            </a>
                            <a href="{{ route('staff.laundries.show', $proof->laundry) }}" class="btn btn-outline-secondary" style="border-radius: 10px; font-size: 0.85rem; padding: 0.65rem;">
...
<div class="modal fade pv-modal" id="bulkApproveModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form method="POST" action="{{ route('staff.payments.verification.bulk-approve') }}" id="bulkApproveForm">
                @csrf
