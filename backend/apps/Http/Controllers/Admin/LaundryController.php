...
class LaundryController extends Controller
{
    public function index(Request $request)
    {
        ...
        $tabCounts = [
            'all'      => Laundry::count(),
            'walkin'   => Laundry::whereNull('pickup_request_id')->count(),
            'delivery' => Laundry::whereHas('pickupRequest', fn($q) => $q->whereIn('service_type', ['both', 'delivery_only']))->count(),
        ];
        ...
        return view('admin.laundries.index', compact('laundries', 'services', 'branches', 'staff', 'stats', 'tab', 'tabCounts'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($validated['service_id'])) {
            ...
            // Check if service has supplies configured — block if none are set up
            $supplies = $service->supplies;
            if ($supplies->isEmpty()) {
                $errors = new MessageBag();
                $errors->add('stock_error', "Cannot create laundry: Service '{$service->name}' has no supplies configured. Please set up required supplies for this service before creating a laundry.");
                return back()->withErrors($errors)->withInput();
            }
            ...
            if ($supplies->isNotEmpty()) {
                ...
                foreach ($supplies as $supply) {
                    ...
                    // Block if no stock record exists OR stock is insufficient
                    if (!$branchStock || $branchStock->current_stock <= 0) {
                        $outOfStock[] = "{$supply->name} (Required: {$quantityRequired} {$supply->distribution_unit}, Available: 0)";
                    } elseif ($branchStock->current_stock < $quantityRequired) {
                        $insufficientStock[] = "{$supply->name} (Required: {$quantityRequired} {$supply->distribution_unit}, Available: {$branchStock->current_stock} {$supply->distribution_unit})";
                    }
                }
        ...
        if (!empty($validated['addons'])) {
            ...
            foreach ($validated['addons'] as $addonEntry) {
                ...
                } elseif ($branchStock->current_stock < $quantity) {
                    $addonStockErrors[] = "{$item->name} has INSUFFICIENT STOCK (Required: {$quantity} {$item->distribution_unit}, Available: {$branchStock->current_stock} {$item->distribution_unit})";
                }
            }
        ...
        // Notify customer about new laundry — handled automatically by LaundryObserver
    ...
    {
        $validated = $request->validate([
            'status' => 'required|in:received,processing,ready,out_for_delivery,delivered,paid,completed,cancelled',
            'notes'  => 'nullable|string|max:500',
        ]);

        $newStatus = $validated['status'];

        $laundry->updateStatus($newStatus, Auth::user(), $validated['notes'] ?? null);

        // Set extra timestamps not handled by updateStatus()
        $extraTimestamps = [
            'out_for_delivery' => 'out_for_delivery_at',
            'delivered'        => 'delivered_at',
        ];
        if (isset($extraTimestamps[$newStatus]) && !$laundry->{$extraTimestamps[$newStatus]}) {
            $laundry->update([$extraTimestamps[$newStatus] => now()]);
        }

        // Notification handled automatically by LaundryObserver

        return redirect()->route('admin.laundries.show', $laundry)
            ->with('success', 'Laundry status updated to: ' . $laundry->fresh()->status_label);
    }
    ...
    public function recordPayment(Request $request, Laundry $laundry)
    {
        DB::transaction(function () use ($request, $laundry) {
            ...
            if ($laundry->isDeliveryOrder()) {
                ...
                // Notification handled automatically by LaundryObserver (paid status)
            }
